...
import {
  focusDashboardBackHref,
  focusDashboardGuardsHref,
  focusDashboardSitesHref,
  isFocusDashboardPath,
} from "@/lib/portal-focus";
...
function FocusPortalShellInner({ user, children }: FocusPortalShellProps) {
  ...
  const showFocusNav = isFocusDashboardPath(pathname);
  const sitesHref = focusDashboardSitesHref(user.role);
  const guardsHref = focusDashboardGuardsHref(user.role);
    ...
    <div className="lunar-page-bg flex min-h-dvh flex-col">
      ...
      <header className="sticky top-0 z-40 flex shrink-0 items-center gap-2 border-b border-[var(--portal-border)] bg-[var(--portal-header-bg)] px-3 py-2.5 shadow-sm backdrop-blur-md sm:gap-3 sm:px-4">
        <div className="flex shrink-0 flex-wrap items-center gap-1.5 sm:gap-2">
          ...
          {showFocusNav ? (
            <>
              <Link href={sitesHref} className="lunar-btn-secondary lunar-btn-sm shrink-0 whitespace-nowrap">
                Back to Sites
              </Link>
              <Link href={guardsHref} className="lunar-btn-secondary lunar-btn-sm shrink-0 whitespace-nowrap">
                Back to Guards
              </Link>
            </>
          ) : null}
